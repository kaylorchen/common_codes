...
class Register {
 public:
  ...
  using CallbackFunction = void (*)(Args...);
...
//  reg.RegisterCallback("callback2", function2);
  // 注册回调函数
  reg.RegisterCallback("callback1", callbackFunction1);
  reg.RegisterCallback("callback2", callbackFunction2);

  // 运行回调函数
  reg.RunCallback<int, double>("callback1", 10, 3.14);
  ...
  reg.RunCallback<const std::string &, int, float>("callback2", test, 42, 2.71f);

  // 尝试运行一个未注册的回调函数
  reg.RunCallback<int>("callback3", 42);
